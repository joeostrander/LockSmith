...
namespace Trinet.Networking
{
    ...
    public class ShareCollection : ReadOnlyCollectionBase
    {
        ...
        public static string UncPathToRemoteFilePath(string uncFilePath)
        {
            ...
            if (string.IsNullOrEmpty(uncFilePath)) return null;
            if (!uncFilePath.StartsWith(@"\\")) return null;
            if (!System.IO.File.Exists(uncFilePath)) return null;
            ...
            if (string.IsNullOrEmpty(server) || string.IsNullOrEmpty(share) || string.IsNullOrEmpty(remainder)) return null;
